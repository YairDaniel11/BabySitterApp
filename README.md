...
### עדכון: תיקון באג חיוג חוזר, שיפור זיהוי רעש, "אין מענה" חכם יותר, אייקון וחיווי פוקוס
- **תוקן באג מרכזי**: אחרי חיוג ראשון, ההאזנה לפעמים "נתקעה בשקט" ולא
  ...
  שניות אם לא - במקום להסתמך רק על callbacks/חריגות שלא תמיד מגיעים בפועל
  בכל מכשיר. בנוסף, ההקלטה נעצרת כעת באופן יזום ברגע שמתחילה שיחה (RINGING/
  OFFHOOK) במקום להישאר רצה על עיוור. ראו הערות מפורטות ב-`CryDetectionService`
  (`WATCHDOG_*`, `scheduleWatchdog()`, `pauseRecordingForCall()`).
- **שיפור זיהוי רעש**: כעת נדרשות 2 דגימות רצופות מעל הסף (כ-1.4 שניות של
  רעש מתמשך) לפני שמתבצע חיוג, כדי לצמצם חיוגי שווא מרעש חד-פעמי (טריקת
  דלת, שיעול). נוסחת הסף עצמה (`1500 + sensitivityPercent * 130`) נבדקה
  ונשארה ללא שינוי - סבירה כערך התחלתי, אך מומלץ לכוונן בהתאם לתנאים
  בפועל בחדר.
- **"אין מענה" / זיהוי דחייה לתא קולי**: נוסף איתות נוסף (`FAST_CONNECT_SUSPICIOUS_MS`)
  שמזהה מעבר מהיר מדי למצב OFFHOOK כחשוד לדחייה מיידית שהועברה לתא קולי,
  בנוסף לבדיקת משך השיחה הקיימת - ומחייג למספר המשני אם הוגדר. **חשוב:**
  Android לא חושף API רשמי שמבדיל בוודאות "ההורה ענה" מ"תא קולי" לאפליקציה
  שאינה החייגן ברירת המחדל - זו עדיין הערכה (heuristic) משופרת, לא זיהוי
  ודאי. ראו הערה מלאה מעל `FAST_CONNECT_SUSPICIOUS_MS` ב-`CryDetectionService`.
- **חיווי פוקוס ירוק לכפתורי "אנשי קשר"**: נוסף `res/drawable/bg_contact_button.xml`
  (selector) עם מסגרת ירוקה דקה (`@color/focus_green`, זהה לצבע ה-tint
  הסטנדרטי של הפלטפורמה לצ'קבוקס מסומן) כשהכפתור בפוקוס - חיווי ברור
  לניווט עם מקשים פיזיים/D-pad.
- **אייקון האפליקציה הוחלף** בכל צפיפויות המסך (כולל round ו-adaptive icon)
  לגרסה ללא סרט "TRIAL".
